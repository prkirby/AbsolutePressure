...
if ( class_exists('WP_Customize_Control' ) ) {
    	class Corpobox_Category_Control extends WP_Customize_Control {
        		public function render_content() {
            			$dropdown = wp_dropdown_categories(
                				array(
                    					'name'              => '_customize-dropdown-categories-' . $this->id,
                    					'echo'              => 0,
                    					'show_option_none'  => __( '&mdash; Select &mdash;', 'corpobox' ),
                    					'option_none_value' => '0',
                    					'selected'          => $this->value(),
                				)
            			);
 ...
 //CONTROL
	...
	$wp_customize->add_section(
		...
		array(
			...
			'description' => __( 'These options are valid for Home Main and Home One page templates. Into Tagline Text you can use the shortcode of the plugin (e.g. slider, google map, etc.).', 'corpobox' ),
			'priority'  => 70
		)
	);
	$wp_customize->add_setting(
	...
	$wp_customize->add_section(
		'corpobox_home_page',
		array(
			'title'     => __( 'Home Page Options', 'corpobox' ),
			'description' => __( 'These options are valid for Home Main page template.', 'corpobox' ),
			'priority'  => 75
		)
	);
	$wp_customize->add_setting( 
		'corpobox_display_pagetitle',
		array(
			'default'     => false,
			'sanitize_callback' => 'corpobox_sanitize_checkbox',
			'transport'   => 'refresh'
	...
	$wp_customize->add_control(
		'corpobox_display_pagetitle',
		array(
			'section'   => 'corpobox_home_page',
			'label'     => __( 'Hide Page Title', 'corpobox' ),
			'type'      => 'checkbox'
		)
	);
	/* Category */
	$wp_customize->add_setting( 'corpobox_category_set', array(
            				'default'        => '0',
				'sanitize_callback'  => 'absint',
				'transport'   => 'refresh'
        		) );
	$wp_customize->add_control(
    		new Corpobox_Category_Control(
        				$wp_customize,
        				'corpobox_category_set',
       				 array(
					'label'    => __( 'Recent Posts Category', 'corpobox' ),
            					'settings' => 'corpobox_category_set',
            					'section'  => 'corpobox_home_page'
        				)
    		)
	);
	$wp_customize->add_setting(
		'corpobox_recentposts_title',
		array(
			'default'            => __( 'Recent Posts', 'corpobox' ),
	...
	$wp_customize->add_control(
		...
		array(
			'section'  => 'corpobox_home_page',
			'label'    => __( 'Recent Posts Title', 'corpobox' ),
			'type'     => 'text'
		)
	);
	$wp_customize->add_setting(
	...
	$wp_customize->add_control(
		...
		array(
			'section'  => 'corpobox_home_page',
			'label'    => __( 'Number Posts on Home', 'corpobox' ),
			'type' => 'select',
			'choices' => array(
				'3' => '3',
				'6' => '6',
...
//Call to Action CONTROL
	 ...
	 *-----------------------------------------------------------*/
	$wp_customize->add_section(
		'corpobox_footer_options',
		array(
			'title'     => __( 'Footer Options', 'corpobox' ),
			'priority'  => 600
		)
	);
	$wp_customize->add_setting(
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'corpobox_footer_bg', array(
		'label' => __( 'Footer BG image', 'corpobox' ),
		'section' =>  'corpobox_footer_options',
		'settings' => 'corpobox_footer_bg'
	) ) );
	$wp_customize->add_setting(
		'copyright_txt',
		array(
	...
	$wp_customize->add_control(
		...
		array(
			'section'  => 'corpobox_footer_options',
			'label'    => __( 'Copyright text', 'corpobox' ),
			'type'     => 'text'
		)
	);
	/*-----------------------------------------------------------*
...
.header-cart li a { background: <?php echo esc_attr( get_theme_mod( 'corpobox_hover_color', '#f16272' ) ); ?>; }
